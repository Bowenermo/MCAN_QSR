...
from openvqa.models.mcan_qsr.adapter import Adapter
    ...
    def __init__(self, __C):
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "timm is required by mcan_qsr. Install with `pip install timm`."
            ) from exc
...
class QuestionSummaryAttention(nn.Module):
    def __init__(self, __C):
        super(QuestionSummaryAttention, self).__init__()
        hidden = int(getattr(__C, 'QSR_SUMMARY_HIDDEN', __C.HIDDEN_SIZE))
        self.fc = nn.Linear(__C.HIDDEN_SIZE, hidden)
        self.score = nn.Linear(hidden, 1)

    def forward(self, lang_feat, lang_mask):
        # lang_feat: [B, L, H], lang_mask: [B,1,1,L], True means pad.
        valid = (~lang_mask.squeeze(1).squeeze(1)).float()
        logits = self.score(torch.tanh(self.fc(lang_feat))).squeeze(-1)
        ...
        q_vec = torch.sum(alpha.unsqueeze(-1) * lang_feat, dim=1)
        return q_vec


class QuestionSoftRegionSelector(nn.Module):
    def __init__(self, __C):
        super(QuestionSoftRegionSelector, self).__init__()
        self.__C = __C
        hidden = int(getattr(__C, 'QSR_SELECTOR_HIDDEN', __C.HIDDEN_SIZE))
        num_regions = int(getattr(__C, 'QSR_NUM_REGIONS', 48))
        self.num_regions = num_regions

        self.q_summary = QuestionSummaryAttention(__C)

        self.objectness = nn.Sequential(
            nn.Linear(__C.HIDDEN_SIZE, hidden),
            nn.GELU(),
            nn.Linear(hidden, 1),
        )
        self.v_proj = nn.Linear(__C.HIDDEN_SIZE, hidden)
        self.q_proj = nn.Linear(__C.HIDDEN_SIZE, hidden)
    ...
    def _current_tau(self):
        # 0-based epoch index: epoch0->tau1, epoch1->tau2, later->tau3
        if self.current_epoch <= 0:
            return float(getattr(self.__C, 'QSR_TAU_EPOCH1', 2.0))
        if self.current_epoch == 1:
            return float(getattr(self.__C, 'QSR_TAU_EPOCH2', 1.0))
        return float(getattr(self.__C, 'QSR_TAU_LATE', 0.7))

    def _diversity_loss(self, assign):
        # assign: [B, K, N]
        bsz, k_num, _ = assign.size()
    ...
    def forward(self, dense_tokens, patch_pos, lang_feat, lang_mask):
        # dense_tokens: [B,N,H], patch_pos: [B,N,5]
        q_vec = self.q_summary(lang_feat, lang_mask)  # [B,H]

        obj = self.objectness(dense_tokens)  # [B,N,1]
        v_proj = self.v_proj(dense_tokens)   # [B,N,H]
        q_proj = self.q_proj(q_vec).unsqueeze(1)  # [B,1,H]
        p_proj = self.p_proj(patch_pos)  # [B,N,H]

        fused = F.gelu(v_proj + q_proj + p_proj)
        score = self.score_head(fused)  # [B,N,K]
        score = score + float(getattr(self.__C, 'QSR_LAMBDA_OBJ', 0.2)) * obj

        tau = self._current_tau()
        assign = F.softmax(score.transpose(1, 2) / tau, dim=-1)  # [B,K,N]
...
class Net(nn.Module):
    def __init__(self, __C, pretrained_emb, token_size, answer_size):
        ...
        self.selector = QuestionSoftRegionSelector(__C)
    ...
    def forward(self, frcn_feat, grid_feat, bbox_feat, ques_ix):
        ...
        img_feat, img_feat_mask, div_loss = self.selector(
            dense_tokens,
            patch_pos,
            lang_feat,
            lang_feat_mask
